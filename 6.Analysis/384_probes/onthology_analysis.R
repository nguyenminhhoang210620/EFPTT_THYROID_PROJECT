...
# input data 
data <- as.data.frame(fread(paste0(path, '/Data/trainndata_dif.tsv'), header = T))

# Call CpGs 
cpgorders <- colnames(data)[1:384]
...
dim(topgst.bias)
write.table(topgst.bias, paste0(path,"/Result/gene_onthology/ontology_GO.tsv"), sep = "\t", col.names = T, row.names = F, quote = F)
...
dim(topkeggsig)
write.table(topkeggsig, paste0(path,"/Result/gene_onthology/ontology_KEGG.tsv.tsv"), sep = "\t", col.names = T, row.names = F, quote = F)
...
# loadind data 
GO <- as.data.frame(fread(paste0(path, "/Result/gene_onthology/ontology_GO.tsv")))
table(GO$ONTOLOGY)
#BP   CC   MF 
#1060  130  245
...
write.table(BP, paste0(path,"/Result/gene_onthology/ontology_GO_BP.tsv"), sep = "\t", col.names = T, row.names = F, quote = F)
write.table(CC, paste0(path,"/Result/gene_onthology/ontology_GO_CC.tsv"), sep = "\t", col.names = T, row.names = F, quote = F)
write.table(MF, paste0(path,"/Result/gene_onthology/ontology_GO_MF.tsv"), sep = "\t", col.names = T, row.names = F, quote = F)
