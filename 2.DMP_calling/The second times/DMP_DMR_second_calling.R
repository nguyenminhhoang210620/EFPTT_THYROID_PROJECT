...
# creat path 
data_dir <- "/home/minhhoang/Documents/Thyroid cancer/TOTAL/Calling_DMP/Second_calling_DMP"
# config file 
configFile <- paste0(path, "/Data/config/GSE146003.config")
...
data <- as.data.frame(fread(paste0(data_dir,"/Data/trainndata_dif.tsv")),header=T)
tdata <- as.data.frame(t(data[,-c(385,386)]))
...
### prepare design.cs
targets <- as.data.frame(fread(paste0(data_dir,'/Data/traindata_info.tsv'), header=T))
info <- targets[,c("title","subtype")]
designTSV <- info
names(designTSV) <- c("title", "groups")
write.table(designTSV, paste0(data_dir, "/design.tsv"), sep="\t", quote=F, row.names=F)


-------------------------------------------------
#NIFTP
designTSV$NIFTP <- ifelse(designTSV$groups=="NIFTP","NIFTP","non-NIFTP")
names(designTSV) <- c("title","group","groups")
write.table(designTSV, paste0(data_dir, "/Result/design_NIFTP.tsv"), sep="\t", quote=F, row.names=F)
----------
info <- targets[,c("title","subtype")]
designTSV <- info
names(designTSV) <- c("title", "groups")
designTSV$FA <- ifelse(designTSV$groups=="FA","FA","non-FA")
names(designTSV) <- c("title","group","groups")
write.table(designTSV, paste0(data_dir, "/Result/design_FA.tsv"), sep="\t", quote=F, row.names=F)
----------------
info <- targets[,c("title","subtype")]
designTSV <- info
names(designTSV) <- c("title", "groups")
designTSV$FC <- ifelse(designTSV$groups=="FC","FC","non-FC")
names(designTSV) <- c("title","group","groups")
write.table(designTSV, paste0(data_dir, "/Result/design_FC.tsv"), sep="\t", quote=F, row.names=F)
-----------
info <- targets[,c("title","subtype")]
designTSV <- info
names(designTSV) <- c("title", "groups")
designTSV$fvPTC <- ifelse(designTSV$groups=="fvPTC","fvPTC","non-fvPTC")
names(designTSV) <- c("title","group","groups")
write.table(designTSV, paste0(data_dir, "/Result/design_fvPTC.tsv"), sep="\t", quote=F, row.names=F)
